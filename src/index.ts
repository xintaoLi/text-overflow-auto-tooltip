...
export const setTextOverflowTooltip = (options: ITextoverflowOption) => {
    ...
    if (typeof tooltip_target === 'string') {
      ...
      if (Array.isArray(options.contentAttributes)) {
        ...
        while (i < options.contentAttributes.length) {
          ...
          if (target[attr] !== undefined && target[attr] !== null && target[attr] !== '') {
            result = target[attr];
          }
  ...
  const showTooltip = () => {
    ...
    if (options.showTooltip && !/true/.test(`${system_title}`)) {
      ...
      targetValue =
        (tooltip_title as string) || (targetNode as HTMLInputElement).value || (targetNode as HTMLElement)?.innerText;
...
export function copyCssStyleToTargetNode(
  ...
  if (propertyList.length > 0) {
    const cssText = propertyList.reduce(
